...
export async function GET(request, { params }) {
  try {
    ...
    const formattedEntries = entries.map((entry) => ({
      id: entry.id,
      name: entry.userData.name || entry.user?.name || "Anonymous",
      email: entry.userData.email || entry.user?.email || "",
      ...
      notes: entry.userData.notes || "",
      groupSize: entry.userData.groupSize || 1,
